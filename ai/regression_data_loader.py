    ...
    def load_dataset(self, args, mode):
        ...
        for value in tqdm(data_list):
            ...
            for idx_area in range(start_idx, 9):
                ...
                # 128×128 고정 통일 (make_double/split-forward 폐기).
                # 가변패치+반쪽 forward가 BatchNorm running stat을 오염시켜
                # train(배치stat)↔eval(running stat) 불일치 유발 → 전 area
                # 동일하게 res×res resize. 학습/추론과 동일 전처리.
